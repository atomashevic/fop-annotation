...
# Save the results
output_file = f'results/appleCLIP-NE-emotion_{start_index + 1}-{end_index}.csv'
...
if end_index == total_images:
    ...
    for i in range(0, total_images, batch_size):
        batch_file = f'results/appleCLIP-NE-emotion_{i + 1}-{min(i + batch_size, total_images)}.csv'
        if os.path.exists(batch_file):
            batch_df = pd.read_csv(batch_file)
            all_df = pd.concat([all_df, batch_df], ignore_index=True)
            os.remove(batch_file)  # Delete the intermediary file
    
    final_output = 'results/appleCLIP-NE.csv'
