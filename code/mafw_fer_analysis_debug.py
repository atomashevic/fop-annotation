...
TEMP_DIR = 'temp_debug'
DATA_DIR = 'data/mafw_sample/videos/'  # Updated path to sample videos
...
def main():
    try:
        ...
        if gpus:
            ...
            logging.warning('No GPU devices found, processing may be slower')
            logging.debug(f'CUDA available: {tf.test.is_built_with_cuda()}')

        # Get list of all MAFW videos from sample directory
        all_videos = []
        ...
        logging.info(f"Found {len(all_videos)} videos in sample directory")
